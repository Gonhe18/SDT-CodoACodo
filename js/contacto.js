...
const validarCampo = (expresion, input, campo) => {
    if (expresion.test(input.value)){
        document.getElementById(`grupo_${campo}`).classList.remove("bloque_campos-incorrecto");
        document.getElementById(`grupo_${campo}`).classList.add('bloque_campos-correcto');
        ...
        document.getElementById(`grupo_${campo}`).classList.add("bloque_campos-incorrecto");
        document.getElementById(`grupo_${campo}`).classList.remove('bloque_campos-correcto');
...
form_nuevo.addEventListener('submit', (e) => {
    ...
    if(campos.apellido&&campos.nombre&&campos.email&&campos.tel&&campos.tramite&&campos.asunto&&campos.comentario){
        form_nuevo.reset();
        document.getElementById('form_msjExito').classList.add('form_msjExito-activo');
        document.getElementById('form_msje').classList.remove('form_msje-activo');
        setTimeout(() => {
            document.getElementById('form_msjExito').classList.remove('form_msjExito-activo');
        }, 1200);

        document.querySelectorAll('.bloque_campos-correcto').forEach((icono)=>{
